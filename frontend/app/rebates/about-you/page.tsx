...
export default function AboutYouPage() {
    ...
    const [incomeBracket, setIncomeBracket] =
        useState<IncomeBracket>("over_80k");
                ...
                <div className="w-full max-w-3xl space-y-8">
                    <div>
                        ...
                        <p className="mt-3 text-[15px] text-[var(--muted)]">
                            We use these to filter which programs you might
...
function OutsideOntarioCard({ postal }: { postal: string | null }) {
    return (
        <div className="rounded-3xl border border-[var(--border)] bg-[var(--card)] p-8 text-center">
            ...
            <p className="mt-3 text-[15px] text-[var(--muted)]">
                Helios&apos;s rebate engine is tuned to Ontario&apos;s incentive
                landscape (HRSP, OEB net metering, Save on Energy). The postal
                code on your bill{postal ? ` (${postal})` : ""} resolves to
                another province, so the program list wouldn&apos;t be honest.
            </p>
            <p className="mt-3 text-[15px] text-[var(--muted)]">
